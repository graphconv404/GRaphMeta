...
def scnet50(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        save_model = torch.load('./scnet50true.pth')
...
def scnet50_v1d(pretrained=True, **kwargs):
    ...
    model = SCNet(SCBottleneck, [3, 4, 6, 3],
                   ...
                   avd=True, **kwargs)
    if pretrained:
        # model.load_state_dict(model_zoo.load_url(model_urls['scnetv50true']))
        
        save_model = torch.load('./scnetv50true.pth')
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in save_model.items() if k in model_dict.keys()}
        model_dict.update(state_dict)
        model.load_state_dict(model_dict)
